...
int video_display(int cpuid, int video_index, int crtc_index, int plane_index, uint32_t display_x, uint32_t display_y, uint32_t display_w, uint32_t display_h)
{
    ...
    while(!_terminate)
    {
        srcBuffer = get_img(dequeue, buffer, srcBuffer);
    ...
    ret = drmModeSetCrtc(dev->fd, save_crtc->crtc_id, save_crtc->buffer_id, save_crtc->x, save_crtc->y, &save_connector->connector_id, 1, &save_crtc->mode);
    if (ret) {
